...
--   molcasDriver getter opts (MolcasTinker molcasInput molcasQM) temp $ addExternalVec initialMol vel
  driverNVT getter opts (MolcasTinker molcasInput molcasQM) (getter getProject) temp $ addExternalVec initialMol vel
...
-- | Using the energies of the states in each integration it is calculated a Histogram
processHisto :: Options -> IO ()
processHisto opts = do
  let file@[salida] =  optInput opts 
  rs <- readSalida salida
  writeFile "gap.out" $ concatMap (printf "%12.8f\n") rs

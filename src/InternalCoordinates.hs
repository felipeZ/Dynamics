...
calcDerv vss numat q = VU.concat $ fmap filler [0..pred numat]
  ...
  where ind x    = vss V.! (pred x ) -- atomic index begins at 1    
        ...
        (ixs,dervs) = case q of
                        Bond     a b     -> ([a,b],derv_bond (ind a) (ind b)) 
                        Angle    a b c   -> ([a,b,c],derv_angle (ind a) (ind b) (ind c))                                           
                        Dihedral a b c d -> ([a,b,c,d],derv_dih (ind a) (ind b) (ind c) (ind d)) 
...
--     numat = len(mol); ndim = len(bond) + len(ang) + len(dih)
--     wilson = np.zeros((ndim,3*numat), dtype = float)
-- 
--     for i in range(len(bond)):
--         m1 = bond[i][0]; n1=3*(m1-1)
--         m2 = bond[i][1]; n2=3*(m2-1)
--         wilson[i,n1:n1+3] = derv_bond(mol[m1-1].cart,mol[m2-1].cart)
--         wilson[i,n2:n2+3] = derv_bond(mol[m2-1].cart,mol[m1-1].cart)
-- 
--     for i in range(len(ang)):
--         m1 = ang[i][0]; m2 = ang[i][1]; m3 = ang[i][2] ; j = i + len(bond)
--         n1 = 3*(m1-1);  n2 = 3*(m2-1);  n3 = 3*(m3-1)
...
--         m1 = dih[i][0]; m2 = dih[i][1]; m3 = dih[i][2]; m4 = dih[i][3]; j = i+len(bond)+len(ang)
--         n1 = 3*(m1-1); n2 = 3*(m2-1); n3 =3*(m3-1); n4 = 3*(m4-1)
--         wilson[j,n1:n1+3], wilson[j,n2:n2+3], wilson[j,n3:n3+3], wilson[j,n4:n4+3] = \
--         derv_dih(mol[m1-1].cart,mol[m2-1].cart,mol[m3-1].cart,mol[m4-1].cart)
-- 
